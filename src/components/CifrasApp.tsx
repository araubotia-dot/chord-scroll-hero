...
import { Song, Setlist, DBShape, loadDB, saveDB } from '@/lib/data-storage';
import { NOTES_SHARP, normalizeNote, Note } from '@/lib/music-utils';
...
export default function CifrasApp() {
  ...
  useEffect(()=>{ 
    setEditDraft(selectedSong ? { ...selectedSong } : null); 
    setTranspose(0); 
  }, [selectedSongId]);
  ...
  function newSong() {
    ...
    const s: Song = { 
      ...
      categories: [], 
      key: "C", 
      content: "# Verso\n[C]Letra com [G]acordes em [Am]colchetes", 
      ownerId: db.user.id, 
      createdAt: now, 
      updatedAt: now 
    };
  ...
  function addToSetlist(songId: string, setlistId?: string) {
    ...
    setDb(d => ({ 
      ...
      setlists: d.setlists.map(s => 
        s.id === id && !s.songIds.includes(songId) 
          ? { ...s, songIds: [...s.songIds, songId], updatedAt: Date.now() } 
          : s
      ) 
    }));
  }
  ...
  function moveInSetlist(songId: string, dir: -1 | 1) {
    ...
    const i = arr.indexOf(songId); 
    if (i < 0) return;
    const j = Math.max(0, Math.min(arr.length - 1, i + dir));
    arr.splice(i, 1); 
    arr.splice(j, 0, songId);
    setDb(d => ({ 
      ...d, 
      setlists: d.setlists.map(s => 
        s.id === currentSetlist.id 
          ? { ...s, songIds: arr, updatedAt: Date.now() } 
          : s
      ) 
    }));
  }
  ...
  function nextSong() {
    ...
    if (nextId) { 
      ...
      setTranspose(0); 
      setTimeout(()=>{ showRef.current?.scrollTo({ top: 0 }) }, 0); 
    }
  }
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 grid grid-cols-12 gap-6">
        ...
        <section className="col-span-12 lg:col-span-9 space-y-6">
                ...
                <Card>
                  <CardContent className="p-4 space-y-4">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <select 
                        value={editDraft.key} 
                        onChange={e=>setEditDraft({ ...editDraft, key: normalizeNote(e.target.value) as Note })} 
                      ...
                      <Input 
                        ...
                        onKeyDown={(e)=>{
                          ...
                          if (e.key === "Enter" && v) { 
                            setEditDraft({
                              ...editDraft, 
                              categories: Array.from(new Set([...(editDraft.categories||[]), v]))
                            }); 
                        ...
                        <Badge key={tag} variant="secondary" className="gap-1">
                          ...
                          <button 
                            ...
                            onClick={()=>setEditDraft({
                              ...editDraft, 
                              categories: editDraft.categories.filter(t => t!==tag)
                            })}
                          >
                            ×
                          </button>
                    ...
                    <div className="text-xs text-muted-foreground">
                      Dica: Formato ChordPro — acorde entre [colchetes], comentários iniciam com #.
                    </div>
                  </CardContent>
                </Card>
              </div>
              ...
              <div className="col-span-12 lg:col-span-6 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <div className="w-8 text-center font-mono">{transpose}</div>
                    <Button size="sm" variant="outline" onClick={()=>setTranspose(t=>t+1)}>
                      +
                    </Button>
                ...
                <Card>
                  <CardContent className="p-4 h-[500px] overflow-auto bg-gradient-surface">
                    <ChordRenderer 
                ...
                <Button 
                  onClick={()=>{ 
                    setDb(d=>({ 
                      ...d, 
                      songs: d.songs.map(s=> s.id===editDraft.id ? { ...editDraft } : s) 
                    })); 
          ...
          {view === "show" && selectedSong && (
            <Card className="overflow-hidden shadow-performance">
              {/* Barra superior de controle */}
              <div className="sticky top-0 z-20 bg-card/90 backdrop-blur border-b border-border px-6 py-4 flex items-center gap-4">
                ...
                <div className="ml-auto flex items-center gap-3 text-sm">
                  ...
                  <Button onClick={prevSong} size="sm" variant="outline">
                    ◀︎
                  </Button>
                  <Button onClick={nextSong} size="sm" className="bg-gradient-primary hover:opacity-90">
                    Próxima ▶︎
                  </Button>
                </div>
              </div>
              ...
              <div 
                ref={showRef} 
                className="h-[70vh] lg:h-[75vh] overflow-y-auto px-8 py-8 text-performance-lg bg-gradient-performance"
              >
                <ChordRenderer 
              ...
              {/* Barra inferior: setlist navigation */}
              {currentSetlist && (
                <div className="border-t border-border bg-muted/30 px-6 py-4 flex items-center gap-3 text-sm">
      ...
      <footer className="max-w-7xl mx-auto px-4 py-8 text-xs text-muted-foreground">
        <div className="text-center">
          ...
          Para edição colaborativa, conecte um backend com autenticação.
        </div>
      </footer>
    </div>
  );
